...
/**
 * nm_setting_bond_get_device_name
 * @setting: the #NMSettingBond
 *
 * Returns: the #NMSettingBond:device-name property of the setting
 **/
const char *
nm_setting_bond_get_device_name (NMSettingBond *setting)
{
	g_return_val_if_fail (NM_IS_SETTING_BOND (setting), 0);

	return NM_SETTING_BOND_GET_PRIVATE (setting)->device_name;
}
...
verify (NMSetting *setting, GSList *all_settings, GError **error)
{
	...
	if (!priv->device_name || !strlen(priv->device_name)) {
		g_set_error (error,
				NM_SETTING_BOND_ERROR,
				NM_SETTING_BOND_ERROR_MISSING_PROPERTY,
				NM_SETTING_BOND_DEVICE_NAME);
		return FALSE;
	}
		...
		GValue *value, GParamSpec *pspec)
{
	...
	switch (prop_id) {
		case PROP_DEVICE_NAME:
			g_value_set_string (value, nm_setting_bond_get_device_name (setting));
			break;
		case PROP_MODE:
...
nm_setting_bond_class_init (NMSettingBondClass *setting_class)
{
	...
	g_object_class_install_property
		(object_class, PROP_DEVICE_NAME,
		 g_param_spec_string (NM_SETTING_BOND_DEVICE_NAME,
			 "DeviceName",
			 "The name of the virtual in-kernel bonding nework device",
			 NULL,
